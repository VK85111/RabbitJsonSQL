...
namespace RabbitMQ_simple_SQL_Solution
{
    internal class Program
    {
        public static void Main(string[] args)
        {
            ...
            //Почему не просто var channel = connection.CreateModel() в стиле 1С?
            {
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    var sql_obj = new SQL_tools();
                    sql_obj.sqlConnection = null;
                    sql_obj.sqlConnection = new SqlConnection(GetConnectionStringByName("SQL_URL"));
                    sql_obj.sqlConnection.Open();
            ...
            string GetConnectionStringByName(string name) //не знаю какой класс поставить, паблик и приват тут не ест
            {
                // Assume failure.
        ...
        public class SQL_tools //Может это уже новый Namespace? Типа своя подсистема.
        {
            public SqlConnection sqlConnection { get; set; }

            public void DeleteFromMS_SQL(Rootobject args)
            {
            ...
            public void SendToMS_SQL(Rootobject args)
            {
